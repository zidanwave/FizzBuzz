...
public class CommandTest {
    ...
    @Test
    public void test_command_list_raw(){
        Assert.assertEquals(new Command(new Schemas("g:list.str"),"-g this,is,a,list").getRawValue("g"), "this,is,a,list");
        Assert.assertEquals(new Command(new Schemas("hi:list.int"),"-hi 1,2,3,-4,6").getRawValue("hi"), "1,2,3,-4,6");
    }

    @Test
    public void test_command_g_list(){
        Command command = new Command(new Schemas("g:list.str"), "-g this,is,a,list");
        Assert.assertEquals(Arrays.asList((String[])command.getValue("g")).toString(), "[this, is, a, list]");
    }
    @Test
    public void test_command_hi_list(){
        Command command = new Command(new Schemas("hi:list.int"), "-hi 1,2,3,-4,6");
        Assert.assertEquals((int[])command.getValue("hi"), new int[]{1, 2, 3, -4, 6});
    }

    @Test
    public void test_command_ileagal_args(){
        Assert.assertEquals(new Command(new Schemas("l:bool p:int"),"- -l true").getRawValue("l"), "true");
        Assert.assertNull(new Command(new Schemas("l:bool p:int"),"- - true").getRawValue("l"));
    }
